...
export default function Books() {
  const { books, setBooks, user, members, issued, showAlert, showConfirm } =
    useLibrary();
            ...
            return (
              ...
              >
                <div className="h-56 bg-gray-100 relative overflow-hidden">
                  <span
                    className={`absolute top-3 right-3 px-3 py-1 text-xs font-bold rounded-full z-10 backdrop-blur-sm shadow-sm ${
                      book.available > 0
                  ...
                  <div className="flex gap-2">
                    {isPending ? (
                      ...
                      <Link
                        ...
                        className={`flex-1 flex items-center justify-center py-2.5 rounded-lg text-sm font-medium transition-all shadow-sm ${
                          book.available > 0
                            ? "bg-secondary hover:bg-[#00a38d] text-white cursor-pointer"
                            : "bg-bg-light text-text-muted border border-border-color cursor-not-allowed pointer-events-none"
                        }`}
                      >
                        {book.available > 0
                          ? isAdmin
