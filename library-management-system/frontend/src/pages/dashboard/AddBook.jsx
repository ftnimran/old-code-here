...
import { PREDEFINED_CATEGORIES } from "../../constants/books";

export default function AddBook() {
  const { addBook, user, showAlert } = useLibrary();
    ...
    return (
      <div className="text-center py-20 animate-fade-in bg-bg-white rounded-2xl shadow-sm border border-border-color">
        ...
        <p className="text-text-muted mt-2 mb-8">
          Only administrators have permission to add new books to the library.
        </p>
        <Link
          to="/books"
  ...
  const handlePdfChange = (e) => {
    ...
    if (file) {
      if (file.type !== "application/pdf") {
        showAlert("Please select a PDF file.", "Invalid Format", "error");
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (success) {
      showAlert("Book added successfully!", "Success", "success");
      navigate("/books");
    }
  };
    ...
    <div className="animate-fade-in max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-text-muted mt-1">
          Fill in the details to add a new book to the database.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {isCustomCategory ? (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="w-12 h-12 flex-shrink-0 flex items-center justify-center border border-border-color rounded-lg bg-bg-light text-text-muted hover:bg-danger/10 hover:text-danger hover:border-danger transition-all cursor-pointer"
                  >
          ...
          <div className="flex justify-end gap-4 border-t border-border-color pt-6">
            ...
            <button
              ...
              className={`bg-primary text-white px-6 py-2.5 rounded-lg font-medium shadow-md transition-all flex items-center justify-center gap-2 ${isSubmitting ? "opacity-70 cursor-not-allowed" : "hover:bg-[#5b54e0] cursor-pointer"}`}
            >
              {isSubmitting ? (
