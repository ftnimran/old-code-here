...
export default function AddBook() {
  ...
  if (user?.role !== "admin")
    return (
      <div className="text-center py-20 text-3xl font-bold text-danger animate-fade-in">
        Access Denied
      </div>
    );
  ...
  const handlePdfChange = (e) => {
    ...
    if (file) {
      ...
      // Validation Updated to 10MB
      if (file.size > 10 * 1024 * 1024) {
        alert("PDF size should be less than 10MB (Local Storage limitation).");
        return;
      }
      const reader = new FileReader();
      reader.onloadend = () =>
