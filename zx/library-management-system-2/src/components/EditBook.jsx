...
export default function EditBook() {
  ...
  useEffect(() => {
    ...
    if (targetBook) {
      ...
      // Agar book ki category predefined list me nahi hai, toh input field on kardo
  ...
  const handleSubmit = (e) => {
    ...
    const updatedBooks = books.map((book) => {
      if (book.id === id) {
        ...
        const newAvailable = book.available + qtyDiff;
  ...
  if (!formData) {
    return (
      <div className="text-center py-10 font-medium text-text-muted">
        Loading Book Data...
      </div>
    );
  }
      ...
      <div className="bg-bg-white p-8 rounded-2xl shadow-sm border border-border-color">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* FIXED: Dynamic Category Box for Edit Mode */}
            <div>
              <label className="block text-sm font-medium text-text-dark mb-1.5">
                Category
              </label>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      // Jab band karenge tab ensure karo ki ek predefined set ho jaye
