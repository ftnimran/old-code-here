...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    { name: "Members", path: "/members", icon: "fa-users" },
    { name: "Profile", path: "/profile", icon: "fa-user-circle" },
  ];
  ...
  return (
    <aside className="w-[260px] h-screen bg-bg-white fixed top-0 left-0 z-50 shadow-sm flex flex-col p-6 transition-all duration-300">
      ...
      <div>
        <button
          onClick={logout}
          className="flex items-center gap-3 p-3 rounded-lg font-medium text-danger hover:bg-danger/10 w-full transition-all"
        >
